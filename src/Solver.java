...
public class Solver {
    ...
    public void initGreedy(Data data, Logger logger) {
        System.out.println("Started greedy on " + data.getInfo());
        ...
        logger.emptyLine();
        logger.log(CONSTANTS.getDividerString());
        System.out.println("Greedy finished on: " + data.getInfo());
    }
    ...
    public void ALNS(Data data, Logger logger) {
        ...
        int customerNodeCount = (int) (data.getNodeList().stream().filter(node -> !node.isDepot() && !node.isDumpingSite()).count() * 0.4);
        int noBetterSolutionFound = 0;
        int numberOfNodesToSwap;
        int numberOfSteps = 1;
        int score = 0;
        float T = calculateInitialTemperature(data, CONSTANTS.getW());
        float bestValue = getDataValue(bestData);
        float currentValue, newValue, delta;
        ...
        while (numberOfSteps < 25000 && noBetterSolutionFound < 2000) {
            ...
            System.out.println(numberOfSteps + " of " + data.getInfo() + ", no better solution found in " + noBetterSolutionFound);
            ...
            numberOfNodesToSwap = 4 + (int) (Math.random() * (Math.min(((int) (customerNodeCount * CONSTANTS.getZETA()) - 4), 100) + 1));
        ...
        vehicleAndHeuristicInformation(bestData, logger, heuristicWeightsList, actualValues);

        logger.emptyLine();
        logger.log(CONSTANTS.getDividerString());

        System.out.println("ALNS finished on: " + data.getInfo() + " with " + numberOfSteps + " iterations");
    }
